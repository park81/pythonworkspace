...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        #self.kiwoom_buy_rate() ####################################거래량 급증 추출
    ...
    def condition_event_slot(self): #조건식 루프
    ...
    def condition_slot(self, lRet, sMsg):
        print("호출 성공 여부 %s, 호출결과 메세지 %s" % (lRet, sMsg))

        condition_name_list = self.dynamicCall("GetConditionNameList()")
        print("HTS의 조건식 이름 가져오기 %s" % condition_name_list)
    ...
    def condition_real_slot(self,strCode,strType,strConditionName,strConditionIndex):
        ...
        if strType == "I":
            ...
            print("실시간 등록 코드: %s, 스크린 번호: %s, fid번호: %s " % (strCode,screen_num,fids))
    ...
    def trdata_slot(self, sScrNo,sRQName,sTrCode,sRecordName,sPrevNext):
        ...
        ####################20201201
            ...
            cnt = 0
            for i in range(rows):
                code = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "종목코드") #QString : 다양한 프로그램언어로 되어있는 변수를 통일시켜준다.
                code = code.strip()[1:]
                
                code_nm = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "종목명")
                buy_price = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "현재가")
                ch_before = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "전일대비기호")
                increase_value = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "전일대비")
                learn_rate = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "등락률")
                before_buy = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "이전거래량")
                current_buy = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "현재거래량")
                increase_stock = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "급증량")
                increase_rate = self.dynamicCall("GetCommData(QString,QString,int, QString)",sTrCode,sRQName,i, "급증률")

                if code in self.rate_increase_dict:
                    pass
                else:
                    self.rate_increase_dict.update({code:{}})


                code_nm = code_nm.strip()
                buy_price = int(buy_price.strip())
                value_updown = int(ch_before.strip())
                if value_updown == 2:
                    ch_before = "상승"
                elif value_updown == 5:
                    ch_before = "하락"
                ...
                increase_value = int(increase_value.strip())
                learn_rate = float(learn_rate.strip())
                before_buy = int(before_buy.strip())
                current_buy = int(current_buy.strip())
                increase_stock = int(increase_stock.strip())
                increase_rate = float(increase_rate.strip())

                self.rate_increase_dict[code].update({"종목명":code_nm})
                self.rate_increase_dict[code].update({"현재가":buy_price})
                self.rate_increase_dict[code].update({"전일대비기호":ch_before})
                self.rate_increase_dict[code].update({"전일대비":increase_value})
                self.rate_increase_dict[code].update({"등락률":learn_rate})
                self.rate_increase_dict[code].update({"이전거래량":before_buy})
                self.rate_increase_dict[code].update({"현재거래량":current_buy})
                self.rate_increase_dict[code].update({"급증량":increase_stock})
                self.rate_increase_dict[code].update({"급증률":increase_rate})

                cnt +=1

                print(self.rate_increase_dict[code])
...
##################20201201 My 생성
    def kiwoom_buy_rate(self,code=None,date=None,sPrevNext="0"):
        ...
        self.dynamicCall("SetInputValue(QString,QString)", "시간", "3")
        self.dynamicCall("SetInputValue(QString,QString)", "종목조건", "1")
        self.dynamicCall("SetInputValue(QString,QString)", "가격구분", "8")

        
        self.dynamicCall("CommRqData(QString,QString,int,QString)", "거래량급증요청", "opt10023", sPrevNext,self.screen_calculation_stock) #Tr 서버로 전송 -Transaction
...
############################################################
    ...
    def realdata_slot(self, sCode,sRealType, sRealData):
        ...
        if sRealType =="장시작시간":
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                if asd['매매가능수량'] > 0 and sCode not in self.meme_timing.keys():
                    self.meme_timing[sCode] =0
                #self.meme_timing
                if int(meme_rate) != self.meme_timing[sCode] and int(meme_rate) > 0:
                    if asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate: #가장 높은 퍼센트 가지는 값이 timing에 들어간다.
                        self.meme_timing[sCode] = meme_rate
                # if int(meme_rate) != self.meme_timing[sCode] and int(meme_rate) > 0:
                #     if asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (1 <= meme_rate < 2):
                #         self.meme_timing[sCode] = 1
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (2 <= meme_rate < 3):
                #         self.meme_timing[sCode] = 2
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (3 <= meme_rate < 4):
                #         self.meme_timing[sCode] = 3
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (4 <= meme_rate < 5):
                #         self.meme_timing[sCode] = 4
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (5 <= meme_rate < 6):
                #         self.meme_timing[sCode] = 5
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (6 <= meme_rate < 7):
                #         self.meme_timing[sCode] = 6
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (7 <= meme_rate < 8):
                #         self.meme_timing[sCode] = 7
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (8 <= meme_rate < 9):
                #         self.meme_timing[sCode] = 8
                #     elif asd['매매가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (9 <= meme_rate):
                #         self.meme_timing[sCode] = 9
                
                if asd['매매가능수량'] > 0 and (self.meme_timing[sCode] >= 9):
                    ...
                    if order_success == 0:
                        ...
                        self.meme_timing.pop(sCode)
                    else:
                        print("매도주문 전달 실패")

                elif asd['매매가능수량'] > 0 and (meme_rate - self.meme_timing[sCode] < -0.5) and (1.6< meme_rate < 3): #갭이 -0.8보다 작으면 손절
                    print("%s %s" % ("신규매도를 한다(1.6까지 올라감)", sCode))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,asd['매매가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
                    #list로 안감싸니 데이터가 너무 많아서 각 부분에 데이터가 제대로 할당이 되지 않았다.
                ...
                elif asd['매매가능수량'] > 0 and (meme_rate - self.meme_timing[sCode] < -0.5) and 3<= meme_rate:
                    print("%s %s" % ("신규매도를 한다(3이상감)", sCode))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,asd['매매가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
                    #list로 안감싸니 데이터가 너무 많아서 각 부분에 데이터가 제대로 할당이 되지 않았다.
                ...
                elif asd['매매가능수량'] > 0 and (meme_rate < -1):
                    print("%s %s %0.2f퍼" % ("신규매도를 한다(떨어짐)", sCode, meme_rate))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,asd['매매가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
                    #list로 안감싸니 데이터가 너무 많아서 각 부분에 데이터가 제대로 할당이 되지 않았다.
            ...
            # 오늘 산 잔고에 있을 경우
                ...
                if jd['주문가능수량'] > 0 and sCode not in self.meme_timing.keys():
                    self.meme_timing[sCode] =0
                
                if int(meme_rate) != self.meme_timing[sCode] and int(meme_rate) > 0:
                    if jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate:
                        self.meme_timing[sCode] = meme_rate
                
                # if int(meme_rate) != self.meme_timing[sCode] and int(meme_rate) > 0:
                #     if jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (1 <= meme_rate < 2):
                #         self.meme_timing[sCode] = 1
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (2 <= meme_rate < 3):
                #         self.meme_timing[sCode] = 2
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (3 <= meme_rate < 4):
                #         self.meme_timing[sCode] = 3
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (4 <= meme_rate < 5):
                #         self.meme_timing[sCode] = 4
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (5 <= meme_rate < 6):
                #         self.meme_timing[sCode] = 5
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (6 <= meme_rate < 7):
                #         self.meme_timing[sCode] = 6
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (7 <= meme_rate < 8):
                #         self.meme_timing[sCode] = 7
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (8 <= meme_rate < 9):
                #         self.meme_timing[sCode] = 8
                #     elif jd['주문가능수량'] > 0 and self.meme_timing[sCode] < meme_rate and (9 <= meme_rate):
                #         self.meme_timing[sCode] = 9


                if jd['주문가능수량'] > 0 and (self.meme_timing[sCode] >= 9):
                    print("%s %s" % ("신규매도를 한다(9이상)", sCode))
                ...
                elif jd['주문가능수량'] > 0 and (meme_rate - self.meme_timing[sCode] < -0.5) and (1.6 < meme_rate < 3):
                    print("%s %s" % ("신규매도를 한다(1.6까지올라감)", sCode))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,jd['주문가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
                    #list로 안감싸니 데이터가 너무 많아서 각 부분에 데이터가 제대로 할당이 되지 않았다.
                ...
                elif jd['주문가능수량'] > 0 and (meme_rate - self.meme_timing[sCode] < -0.5) and 3 <= meme_rate:
                    print("%s %s" % ("신규매도를 한다(3이상감)", sCode))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,jd['주문가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
                ...
                elif jd['주문가능수량'] > 0 and (meme_rate < -1):
                    print("%s %s %0.2f퍼" % ("신규매도를 한다(떨어짐)", sCode, meme_rate))
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매도",self.portfolio_stock_dict[sCode]['주문용스크린번호'],self.account_num,2,sCode,jd['주문가능수량'],0,self.realType.SENDTYPE['거래구분']['시장가'],""]) 
                    #주문가격: 시장가로 설정할거라 0을 넣었다. 원주문번호: 처음 주문 번호 (정정하면 주문번호가 바뀌어서 기존 번호 저장해둔곳), 정정 주문시 원주문번호 넣어줘야한다.
                    #주문 넣으면 정상적으로 들어갔는지 바로바로 알기 위해 반환값이 있다. (order_success)
            ...
            #등략율이 2.0% 이상이고 오늘 산 잔고에 없을 경우
            elif sCode in self.add_list and sCode not in self.jango_dict and e !=0:
                print("%s %s" % ("신규매수를 한다", sCode))
                print(self.portfolio_stock_dict[sCode]["주문용스크린번호"])     
                result = (self.use_money * 0.3) / e #가진돈 0.3% / 현재가
                quantity = int(result)
                print(quantity)

                #퍼센트로 매수점 찾을땐 지정가로 해야해서 하기와 같이 한다.
                ...
                #시장가가 아닌 지정가
                #order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)", ["신규매수",self.portfolio_stock_dict[sCode]["주문용스크린번호"],self.account_num,1,sCode,quantity,0,self.realType.SENDTYPE['거래구분']['시장가'],""])
    ...
    def chejan_slot(self,sGubun, nItemCnt, sFIdList):
        ...
        if int(sGubun) == 0:
            ...
            self.jango_dict[sCode].update({"매입단가": buy_quan})
